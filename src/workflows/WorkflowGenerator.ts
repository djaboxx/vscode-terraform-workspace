...
function generateApplyWorkflow(
  ...
  // The apply workflow uses a two-job pattern so that reviewers can see the
  // plan output before approving the apply:
  //   job 1 (plan)  — no environment gate; runs immediately on push/dispatch
  //   job 2 (apply) — environment gate fires HERE; reviewer sees job 1's logs
  //                   before clicking approve, then apply uses the exact plan
  //                   binary uploaded to S3 by job 1.
  const environmentLine = useGhaEnv
    ? `    environment: "${env.name}"`
    : `    # No GHA Environment — flat repo mode (useGhaEnvironment: false)`;
  const planPermissions  = jobPermissions(awsAuthMode, /*pullRequests*/ false);
  const applyPermissions = jobPermissions(awsAuthMode, /*pullRequests*/ false);
  const awsAuthStep = awsAuthStepYaml(actions.awsAuth, awsAuthMode);
  return `# Generated by Terraform Workspace VS Code Extension
# Re-sync: Ctrl/Cmd+Shift+P → "Terraform: Sync Workflows"
# WARNING: Do not edit manually — changes will be overwritten on next sync.
