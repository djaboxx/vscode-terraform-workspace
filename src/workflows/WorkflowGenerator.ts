  ...
  async generateAll(config: WorkspaceConfig): Promise<GeneratedWorkflow[]> {
    ...
    for (const env of getWorkspaces(config)) {
      ...
      results.push({
        ...
        type: 'plan',
        yaml: generatePlanWorkflow(env, actions, stateConf, envBlock, tfVersion, varfile, useGhaEnvs),
      });
      ...
      results.push({
        ...
        environmentName: env.name,
        type: 'apply',
        yaml: generateApplyWorkflow(env, actions, stateConf, envBlock, tfVersion, varfile, useGhaEnvs),
      });
    }
